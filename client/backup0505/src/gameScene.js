...
var gameLayer = cc.Layer.extend({
    ...
    ctor:function(){
        ...
        console.log("res.BackGround_jpg = " + res.BackGround_jpg);
        for ( var k in this.backSprite){
            console.log("k = " + k);
            console.log("g_dealCardBack.k = " + g_dealCardBack[k]);
        }
        ...
        //cc.spriteFrameCache.addSpriteFrames(res.InteractionAnimation_plist,res.InteractionAnimation_png);
        ////var frameCache=cc.spriteFrameCache;
        //cc.spriteFrameCache.addSpriteFrames(res.BiPaiAnimation_plist,res.BiPaiAnimation_png);
    ...
    initMenu:function(){
        ...
        this.genDaoDiMenuItem=new cc.MenuItemToggle(GenDaoDiOfMenuItem,GenDaoDiOnMenuItem,
            this.menuCallbcak_follow_always,this);
    ...
    initOtherMenu:function(){
        ...
        radioMenuItem.y=size.height-radioMenuItemSprite1.getContentSize().height*3/2;


        this.otherMenu=new cc.Menu(settingMenuItem,gameBackMenuItem,chatMenuItem,radioMenuItem);
        this.otherMenu.x=0;
        this.otherMenu.y=0;
    ...
    initPersonalMessageMenu:function(){
        for(var i=0;i<this.players.length;i++){
            if(this.players[i].id!=uid){
                ...
                this.players[i].spritePhotoMobile.addChild(button);
            }
        }
    ...
    initRadioDisplayTxtCliper:function(){
        ...
        this.radioMenuBackground.y=size.height-this.radioMenuBackground.getContentSize().height*3/2;
        this.addChild(this.radioMenuBackground);

        //初始化滚动字幕成员
        g_cliper = new cc.ClippingNode();
        ...
        g_cliper.setStencil(this.drawNode);
        g_cliper.attr({
            x: 80,
        ...
        this.radioMenuBackground.addChild(g_cliper);
    },
    pomelo_on:function(){
        pomelo.on('onReady',function(data){
            console.log(data.user+" is ready");
        ...
        pomelo.on('onChatInGame',function(data){
            ...
            for(var i=0;i<this.players.length;i++){
                if(this.players[i].positionServer==position){
                    ...
                    switch (this.players[i].playerPosition){
                        case 1:
                            labelString.attr({
                                x:this.players[i].spritePhotoMobile.getPositionX(),
                                y:this.players[i].spritePhotoMobile.getPositionY()+
                                this.players[i].spritePhotoMobile.getContentSize().height/2+5,
                                anchorX:0.5,
                                anchorY:0
                            });
                            break;
                    ...
                    labelString.runAction(cc.sequence(new cc.DelayTime(3),cc.hide(),cc.callFunc(function(){
                        labelString.removeFromParent();
                    })));
                    find=true;
                    break;
                }
            ...
            for(var i=0;i<this.players_noPower.length;i++){
                if(this.players_noPower[i].positionServer==position){
                    ...
                    switch (this.players_noPower[i].playerPosition){
                        case 1:
                            labelString.attr({
                                x:this.players_noPower[i].spritePhotoMobile.getPositionX(),
                                y:this.players_noPower[i].spritePhotoMobile.getPositionY()+
                                this.players_noPower[i].spritePhotoMobile.getContentSize().height/2+5,
                                anchorX:0.5,
                                anchorY:0
                            });
                            break;
    ...
    menuCallbackPersonalMessage:function(sender,type){
    ...
    onEnterTransitionDidFinish:function(){
        ...
        cc.audioEngine.playMusic(res.BackgroundMusic_ogg ,true);
    },
